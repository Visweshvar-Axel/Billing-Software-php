...
if (isset($_GET['id'])) {
    ...
    if ($order_result->num_rows > 0) {
        ...
        // 2. Use $order_data instead of $order->fetch_array()

        // Fetch order items
        $items_query = "SELECT o.*, p.name FROM order_items o INNER JOIN products p ON p.id = o.product_id WHERE o.order_id = ?";
        $items_stmt = $conn->prepare($items_query);
        $items_stmt->bind_param("i", $order_id);
        $items_stmt->execute();
        $items_result = $items_stmt->get_result();
    } else {
        // Handle case when no order is found with the provided ID
    }
}
...
// 3. Define $order_number before using it
$order_number = isset($_GET['id']) ? $_GET['id'] : generateOrderNumber();
?>
...
<script>
    ...
    // });
    $('#save_order').click(function() {
        // Resetting values to 0 for Pay Later option
        $('#tendered').val('0').trigger('change');
        $('#discount').val('0').trigger('input');
        $('#after_discount').val('0');
        $('#change').val('0');
